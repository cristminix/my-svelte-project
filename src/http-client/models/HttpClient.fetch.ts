...
import type { HttpClientInterface, HttpClientConfigInterface } from "./HttpClient.interface"

import { HttpRequestType, HttpContentTypes, HttpRequestMethods } from "./Constants"
  ...
  async request<R, P = void>(parameters: HttpRequestParamsInterface<P>): Promise<R> {
    ...
    try {
      switch (requestType) {
        ...
        default: {
          console.warn(`HttpClientFetch: ivalid requestType argument or request type not implemented`)
        }
      }
    } catch (e) {
