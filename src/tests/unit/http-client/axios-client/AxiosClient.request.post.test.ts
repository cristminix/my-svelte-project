...
import { HttpClientAxios, HttpRequestType, type HttpRequestParamsInterface } from "@/http-client"
...
describe("HttpClient: axios-client: request: post", () => {
  ...
  it("should execute post request succesfully", () => {
    vitest.spyOn(axios, "post").mockImplementation(async () => Promise.resolve({ data: `request completed: ${mockRequestParams.endpoint}` } as any))
    httpClient
